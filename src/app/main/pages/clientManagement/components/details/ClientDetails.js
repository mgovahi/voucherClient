...
import FinInfo from "./finInfo/FinInfo";
import SecSettings from "./securitySettings/SecSettings";
import Permissions from "./permissions/Permissions";
import clsx from "clsx";
function ClientDetails({ clientInfo, onCancelClick }) {
  ...
  return (
    <Box>
      ...
      >
        <Tab label={t("CLIENT_INFO")} />
        <Tab label={t("FIN_INFO")} />
        <Tab label={t("SECURITY_SETTINGS")} />
        <Tab label={t("ACCESSES")} />
      </Tabs>
      <Box>
        <Box className={clsx(active == 0 ? "" : "hidden", "py-[22px]")}>
          <ClientInfo info={clientInfo} onCancelClick={onCancelClick} />
        </Box>
        ...
        <Box className={clsx(active == 3 ? "" : "hidden", "py-[22px]")}>
